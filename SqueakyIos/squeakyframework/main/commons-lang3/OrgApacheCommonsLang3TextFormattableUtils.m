...
//  source: /Users/kgalligan/devel-doppl/commons-lang/src/main/java/org/apache/commons/lang3/text/FormattableUtils.java
//
...
#include "OrgApacheCommonsLang3TextFormattableUtils.h"
...
JavaUtilFormatter *OrgApacheCommonsLang3TextFormattableUtils_appendWithJavaLangCharSequence_withJavaUtilFormatter_withInt_withInt_withInt_withChar_withJavaLangCharSequence_(id<JavaLangCharSequence> seq, JavaUtilFormatter *formatter, jint flags, jint width, jint precision, jchar padChar, id<JavaLangCharSequence> ellipsis) {
  OrgApacheCommonsLang3TextFormattableUtils_initialize();
  OrgApacheCommonsLang3Validate_isTrueWithBoolean_withNSString_withNSObjectArray_(ellipsis == nil || precision < 0 || [ellipsis length] <= precision, @"Specified ellipsis '%1$s' exceeds precision of %2$s", [IOSObjectArray arrayWithObjects:(id[]){ ellipsis, JavaLangInteger_valueOfWithInt_(precision) } count:2 type:NSObject_class_()]);
  JavaLangStringBuilder *buf = create_JavaLangStringBuilder_initWithJavaLangCharSequence_(seq);
  if (precision >= 0 && precision < [((id<JavaLangCharSequence>) nil_chk(seq)) length]) {
    ...
    [buf replaceWithInt:precision - [((id<JavaLangCharSequence>) nil_chk(_ellipsis)) length] withInt:[((id<JavaLangCharSequence>) nil_chk(seq)) length] withNSString:[_ellipsis description]];
  }
  jboolean leftJustify = (flags & JavaUtilFormattableFlags_LEFT_JUSTIFY) == JavaUtilFormattableFlags_LEFT_JUSTIFY;
  for (jint i = [buf length]; i < width; i++) {
