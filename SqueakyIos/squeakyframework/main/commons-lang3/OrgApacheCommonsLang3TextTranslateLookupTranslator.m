...
//  source: /Users/kgalligan/devel-doppl/commons-lang/src/main/java/org/apache/commons/lang3/text/translate/LookupTranslator.java
//
...
- (jint)translateWithJavaLangCharSequence:(id<JavaLangCharSequence>)input
  ...
  if ([((JavaUtilHashSet *) nil_chk(prefixSet_)) containsWithId:JavaLangCharacter_valueOfWithChar_([((id<JavaLangCharSequence>) nil_chk(input)) charAtWithInt:index])]) {
    jint max = longest_;
    if (index + longest_ > [input length]) {
  ...
  if (lookup != nil) {
    {
      ...
      while (b__ < e__) {
        ...
        jint sz = [((id<JavaLangCharSequence>) nil_chk(IOSObjectArray_Get(seq, 0))) length];
        if (sz < _shortest) {
          _shortest = sz;
        }
        if (sz > _longest) {
